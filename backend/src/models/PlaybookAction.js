...
const playbookActionSchema = new mongoose.Schema(
  {
    ...
    source: { type: String, enum: ['ai_mitigation', 'manual', 'playbook_template'], default: 'ai_mitigation' },
    playbookRunId: { type: String, index: true },
    stepOrder: { type: Number, default: 0 },
    templateId: { type: mongoose.Schema.Types.ObjectId, ref: 'PlaybookTemplate' },
    templateName: String,
    target: {
      username: String,
      ip: String,
...
playbookActionSchema.methods.toPublicJSON = function toPublicJSON() {
  return {
    ...
    status: this.status,
    source: this.source,
    playbookRunId: this.playbookRunId,
